...
void updateEncoder() {
  ...
  // When spinning clockwise (for example: 10 -> 11), the encoder value is
  // increased
  if (combination == 0b1011 || combination == 0b1101 || combination == 0b0100 ||
      combination == 0b0010) {
    encoderValue++;
  }
  if (combination == 0b1000 || combination == 0b1110 || combination == 0b0111 ||
      combination == 0b0001) {
    encoderValue--;
  }
